<template>
  <div class="tarot-container min-h-screen bg-white">
    <div v-if="!showCardsList && !showResult" class="main-section px-6 py-8">
      ...
      <div class="space-y-2">
        ...
        <div v-if="shuffled && !showResult" class="select-number-section mt-6">
          ...
          >
            {{ selectedSpread === 'single' ? t('tarot.interpretCards') : (threeCardStep === 2 ? t('tarot.interpretCards') : t('tarot.confirmSelection')) }}
          </button>
        </div>
      </div>
    </div>
    ...
    <div v-else-if="showResult" class="result-section min-h-screen px-6 py-8">
      ...
      <div v-if="selectedSpread === 'single'" class="single-result">
        <div class="card-display bg-gray-50 rounded-2xl p-8 text-center mb-8">
          ...
          />
          <h2 class="text-xl font-semibold mb-1">{{ resultCard?.name }}</h2>
        ...
        <div class="reading-content bg-gray-50 rounded-2xl p-6">
          <h3 class="font-semibold mb-4">{{ t('tarot.reading') }}</h3>
      ...
      <div v-else class="three-result">
        <div class="main-card bg-gray-50 rounded-2xl p-6 mb-6 text-center">
          <p class="text-xs text-gray-500 mb-2">{{ t('tarot.coreCard') }}</p>
          <img 
            :src="getCardImage(resultMainCard?.image)" 
            :alt="resultMainCard?.name" 
            class="w-32 mx-auto mb-3"
            :class="{ 'rotate-180': isMainReversed }"
          />
          <h3 class="font-semibold mb-1">{{ resultMainCard?.name }}</h3>
          <p class="text-sm text-gray-500">{{ resultMainCard?.group }} · {{ isMainReversed ? t('tarot.reversed') : t('tarot.upright') }}</p>
        </div>

        <div class="three-cards grid grid-cols-3 gap-3 mb-8">
          <div v-for="(card, idx) in resultThreeCards" :key="idx" class="text-center p-3 bg-gray-50 rounded-xl">
...
const confirmSelection = () => {
  ...
  if (selectedSpread.value === 'single') {
    resultCard.value = tarotStore.getCardById(selectedSingleNumber.value!)
